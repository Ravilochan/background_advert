...
function App() {
  const { jobId, status, progress, segments, processedVideoUrl, setVideoData, updateStatus, setResult } = useStore();
    ...
    <div className="container">
      ...
      {!jobId ? (
        ...
        <div className="progress-section">
          ...
          <p>{status === 'completed' ? 'Processing Complete!' : `Status: ${status} (${progress}%)`}</p>
          ...
          {segments.length > 0 && (
            <div className="results-section">
              <h3>Analyzed Segments</h3>
              <div className="segments-grid">
                {segments.map((segment, idx) => (
                  <div key={idx} className="segment-card">
                    <img src={`http://localhost:3001/temp/${jobId}/${segment.keyframePath.split('/').pop()}`} alt={`Keyframe ${idx}`} />
